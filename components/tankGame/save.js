...
function draw_game() {
    ...
    function draw_elements() {
        function draw_shuts_func(player) {
            draw_shuts(player.shut.to_left);
            draw_shuts(player.shut.to_right);
            draw_shuts(player.shut.to_top);
            draw_shuts(player.shut.to_down);
        }

        function draw_shuts(to) {
            for (let j = 0; j < to.x.length; j++) {
                ctx.drawImage(shut_player1_img, player_1.shut.to.x[j], player_1.shut.to.y[j], player_1.shut.size, player_1.shut.size);
            }
            for (let j = 0; j < player_2.shut.to.x.length; j++) {
                ctx.drawImage(shut_player2_img, player_2.shut.to.x[j], player_2.shut.to.y[j], player_2.shut.size, player_2.shut.size);
            }
        }
    ...
    window.addEventListener("keydown", (e) => {
        if (e.key == `${player.button_shut}`) {
            ...
            if (player.shut.left) {
                ...
                player.shut.to_left.x.push(player.x + player.width * 0.5 - player.shut.size * 0.5);
                player.shut.to_left.y.push(player.y + player.height * 0.5 - player.shut.size * 0.5);

                let timer = setInterval(function() {
                        shuts_trajectory(player.shut.to_left, player.shut.to_left.x);

                        function shuts_trajectory(pl_sh_to, x_or_y) {
                            for (let j = 0; j < x_or_y.length; j++) {
                                x_or_y[j] = x_or_y[j] + player.shut.speed / x_or_y.length;

                                for (let i = 0; i <= wall_arr.x.length; i++) {
                                    if (canvas.width <= pl_sh_to.x[j] || wall_arr.x[i] <= pl_sh_to.x[j] + player.shut.size && pl_sh_to.x[j] <= wall_arr.x[i] + wall_arr.width[i] && wall_arr.y[i] <= pl_sh_to.y[j] + player.shut.size && pl_sh_to.y[j] <= wall_arr.y[i] + wall_arr.height[i]) {
                                        clearInterval(timer);
                                        x_or_y.splice(j, 1);
                                    } else {
                                        hit();
            ...
            if (player.shut.right) {
                ...
                player.shut.to_right.x.push(player.x + player.width * 0.5 - player.shut.size * 0.5);
                player.shut.to_right.y.push(player.y + player.height * 0.5 - player.shut.size * 0.5);

                let timer = setInterval(function() {
                        for (let j = 0; j < player.shut.x.length; j++) {
                            player.shut.to_right.x[j] = player.shut.to_right.x[j] - player.shut.speed / player.shut.to_right.x.length;
            ...
            if (player.shut.up) {
                ...
                player.shut.to_top.x.push(player.x + player.width * 0.5 - player.shut.size * 0.5);
                player.shut.to_top.y.push(player.y + player.height * 0.5 - player.shut.size * 0.5);

                let timer = setInterval(function() {
                        for (let j = 0; j < player.shut.to_top.y.length; j++) {
            ...
            if (player.shut.down) {
                ...
                player.shut.to_down.x.push(player.x + player.width * 0.5 - player.shut.size * 0.5);
                player.shut.to_down.y.push(player.y + player.height * 0.5 - player.shut.size * 0.5);
